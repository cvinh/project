...
class ProjectTaskMergeWizard(models.TransientModel):
    ...
    _description = 'Wizard that prepares task merging'
    ...
    target_project_id = fields.Many2one(
        'project.project', string="Target Project")
    ...
    def merge_tasks(self):
        ...
        self.target_task_id.message_post_with_view(
            self.env.ref('project_task_merge.mail_template_task_merge'),
            values={
                'target': True,
                'tasks': self.task_ids - self.target_task_id
            },
    ...
    def merge_followers(self):
        self.target_task_id.message_subscribe(
            partner_ids=(
                self.task_ids - self.target_task_id).mapped(
                    'message_partner_ids').ids,
            channel_ids=(
                self.task_ids - self.target_task_id).mapped(
                    'message_channel_ids').ids,
        )
    ...
    def default_get(self, fields):
        ...
        result.update({
            ...
            'user_id': assigned_tasks and
            assigned_tasks[0].user_id.id or False,
